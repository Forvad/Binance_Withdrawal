api_key = ""

secret_key = ""

token = 'USDT'  # токен вывода

ntwork = 'MATIC'  # сеть вывода

value = [10, 12]  # Объём вывода мин - мах

Delay = [30, 60]  # задержка в сек

randoms = 2  # 1 рандом целых чисел, 2 - с десятыми

# STG module
api_key_STG = "9Rj4ZIx4i16DOfz4FOFJNOBwQdELDuDLt4Gn45LukJKWBRsHJa4n1SDD1NKQeZRG"

secret_key_STG = "iVdLbySsqSBw0ufKJkWLRASVOrYkXIPph7EYokHb4ESQMQLFhnLXlcn9RNQ514uB"

WITHDRAVAL_NATIVE = True  # вывод натива

WITHDRAVAL_STEABLE = True  # вывод стейбла

network_steable = ['polygon', 'avalanche', 'optimism', 'arbitrum', 'bsc']  # Выбор 1 рандом сети для вывода стейбла

# Сети вывода нейтива 'polygon', 'avalanche', 'optimism', 'arbitrum', 'bsc', 'harmony', 'moonriver'
# Раскидывает во все выбранные сети нейтив
network_native = ['polygon', 'avalanche', 'optimism', 'arbitrum', 'bsc', 'harmony', 'moonriver']

value_steable = [10, 12]  # Объём вывода мин - мах стейбла

value_native = [1, 2]  # Объём вывода мин - мах нейтива в $

Delay_STG = [30, 60]  # задержка в сек
